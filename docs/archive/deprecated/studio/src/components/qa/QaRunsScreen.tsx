...
export function QaRunsScreen({ runs, onRunsChange }: { runs: QaRun[]; onRunsChange: (runs: QaRun[]) => void }) {
  ...
  async function copyQaCommand() {
    await copyText("node tools/visual-qa.mjs --url http://localhost:5175 --name design-os-studio --browser auto --tmpdir .tmp/playwright");
  }
          ...
          <article className="rounded-xl border border-white/10 bg-black/30 p-5">
            <div className="grid gap-4 md:grid-cols-3">
              <div className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
                <p className="text-xs font-black uppercase tracking-[0.1em] text-zinc-500">Playwright</p>
                <div className="mt-2"><StatusPill value={selectedRun.playwrightStatus} /></div>
                <p className="mt-3 text-sm text-zinc-300">Browser mode: {selectedRun.browserRequested}</p>
              </div>
              <div className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
                <p className="text-xs font-black uppercase tracking-[0.1em] text-zinc-500">Capture mode</p>
                <p className="mt-2 text-lg font-black text-mist">{selectedRun.captureMode}</p>
                <p className="mt-3 text-sm text-zinc-300">{selectedRun.screenshotEvidenceRequired ? "390 / 768 / 1440 required" : "Evidence optional"}</p>
              </div>
              <div className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
                <p className="text-xs font-black uppercase tracking-[0.1em] text-zinc-500">Score threshold</p>
                <p className="mt-2 text-lg font-black text-mist">{selectedRun.scoreCurrent} / {selectedRun.scoreTarget}</p>
                <p className="mt-3 text-sm text-zinc-300">Below 80 cannot be done.</p>
              </div>
            </div>

            <div className="mt-4 rounded-lg border border-amber/30 bg-amber/10 p-4">
              <p className="text-sm font-black text-amber">Blocked reason</p>
              <p className="mt-2 text-sm leading-6 text-zinc-300">{selectedRun.blockedReason}</p>
              <p className="mt-2 text-sm leading-6 text-zinc-300">Automated screenshot capture is preferred. Manual app-only screenshot import is acceptable fallback. Desktop screenshots with browser chrome, dock, or editor UI are not valid QA evidence.</p>
            </div>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              <div className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
                ...
                <p className="mt-3 text-sm text-zinc-300">Final status: {selectedRun.doneReportFinalStatus}</p>
              </div>
              <div className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-3">
              <button type="button" onClick={copyQaCommand} className="min-h-11 rounded-lg border border-signal/40 px-4 text-sm font-black text-signal transition hover:bg-signal hover:text-obsidian">
                Copy auto QA command
              </button>
              <CommandBlock command="node tools/playwright-doctor.mjs --url http://localhost:5175 --browser auto" label="Copy doctor" />
              <CommandBlock command="node tools/import-manual-screenshots.mjs --name design-os-studio --390 path/to/390.png --768 path/to/768.png --1440 path/to/1440.png" label="Copy manual import" />
              <CommandBlock command="node tools/visual-qa.mjs --manual --input-dir docs/qa-runs/<run>/screenshots --name design-os-studio" label="Copy manual QA" />
